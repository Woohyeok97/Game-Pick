...
export default async function handler(req, res) {
    ...
    if(req.method == 'PUT') {
        ...
        try {
            ...
            const result = await db.collection('game_content').updateOne({ _id : new ObjectId(req.query) }, { $set : editContent })
    ...
    if(req.method == 'DELETE') {
        ...
        try {
            const contentComments = await db.collection('game_comment').find({ parent : req.query.id }).toArray()
            const commentIds = contentComments.map(item => item._id.toString())
            // 1. 컨텐츠 코멘트 피드백 삭제
            await db.collection('game_comment_feedback').deleteMany({ parent : { $in : commentIds }})

            // 2. 컨텐츠 코멘트 삭제
            await db.collection('game_comment').deleteMany({ parent : req.query.id })

            // 3. 컨텐츠 피드백 삭제
            await db.collection('game_content_feedback').deleteMany({ parent : req.query.id })

            // 4. 컨텐츠 삭제
            const result = await db.collection('game_content').deleteOne({ _id : new ObjectId(req.query.id) })
